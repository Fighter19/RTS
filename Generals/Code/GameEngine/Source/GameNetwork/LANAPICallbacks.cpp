...
void LANAPI::OnPlayerList( LANPlayer *playerList )
{
	if (m_inLobby)
	{
		...
		if (selectedIndex != -1 )
			selectedIP = (UnsignedIntPtr) GadgetListBoxGetItemData(listboxPlayers, selectedIndex, 0);
		...
		while (player)
		{
			...
			GadgetListBoxSetItemData(listboxPlayers, (void *)(UnsignedIntPtr)player->getIP(),addedIndex, 0 );
