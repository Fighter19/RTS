...
/*
      ...
      if((comboColor[i] != NULL) && BitTest(comboColor[i]->winGetStatus(), WIN_STATUS_ENABLED))
			...
			if (comboColor[i] != NULL) {
				...
				{
					Int color = (IntPtr)GadgetComboBoxGetItemData(comboColor[i], idx);
			...
			if (comboTeam[i] != NULL) {
				...
				for (idx=0; idx<max; ++idx)
				{
					Int team = (IntPtr)GadgetComboBoxGetItemData(comboTeam[i], idx);
			...
			if (comboPlayerTemplate[i] != NULL) {
				...
				for (idx=0; idx<max; ++idx)
				{
					Int playerTemplate = (IntPtr)GadgetComboBoxGetItemData(comboPlayerTemplate[i], idx);
