...
void UpdateStartButton(void)
{
	...
	index = (IntPtr)GadgetComboBoxGetItemData( comboBoxLadder, selected );
...
//	pref.setMinPoints(min(100, atoi(a.str())));
	...
	item = (IntPtr)GadgetComboBoxGetItemData(comboBoxSide, selected);
		...
		case GCM_SELECTED:
			{
				...
				if (controlID == comboBoxLadderID && !isPopulatingLadderBox)
				{
					...
					{
						QuickMatchPreferences pref;
						Int ladderID = (IntPtr)GadgetComboBoxGetItemData(control, pos);
						if (ladderID == 0)
						{
...
//					a.translate(u);
					...
					GadgetComboBoxGetSelectedPos( comboBoxLadder, &selected );
					ladderIndex = (IntPtr)GadgetComboBoxGetItemData( comboBoxLadder, selected );
					...
					GadgetComboBoxGetSelectedPos( comboBoxSide, &selected );
					if (selected >= 0)
						index = (IntPtr)GadgetComboBoxGetItemData( comboBoxSide, selected );
					req.QM.side = index;
					...
					GadgetComboBoxGetSelectedPos( comboBoxColor, &selected );
					if (selected >= 0)
						index = (IntPtr)GadgetComboBoxGetItemData( comboBoxColor, selected );
					req.QM.color = index;
