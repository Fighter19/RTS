...
static void populateLadderListBox( void )
{
	...
	if (selIndex < 0)
		return;
	selID = (IntPtr)GadgetListBoxGetItemData(listboxLadderSelect, selIndex);
	if (!selID)
		return;
    ...
    case GBM_SELECTED:
		{
			...
			if (controlID == buttonOkID)
			{
				...
				ladderIndex = (IntPtr)GadgetListBoxGetItemData( listboxLadderSelect, selectPos, 0 );
		...
		case GBM_SELECTED:
			{
				GameWindow *control = (GameWindow *)mData1;
				Int controlID = control->winGetWindowId();
				Int selectedID = (IntPtr)window->winGetUserData();
				if(!selectedID)
					break;
				closeRightClickMenu(window);
