		...
		case GLM_RIGHT_CLICKED:
			{
				...
				if( controlID == buddyControls.listboxBuddiesID ) 
				{
					...
					GPProfile profileID = (GPProfile)(IntPtr)GadgetListBoxGetItemData(control, rc->pos, 0);
					RCItemType itemType = (RCItemType)(IntPtr)GadgetListBoxGetItemData(control, rc->pos, 1);
			...
			case GEM_EDIT_DONE:
			{
				...
				if (selected >= 0)
				{
					GPProfile selectedProfile = (GPProfile)(IntPtr)GadgetListBoxGetItemData(buddyControls.listboxBuddies, selected);
...
void updateBuddyInfo( void )
{
	...
	GadgetListBoxGetSelected(buddyControls.listboxBuddies, &selected);
	if (selected >= 0)
		selectedProfile = (GPProfile)(IntPtr)GadgetListBoxGetItemData(buddyControls.listboxBuddies, selected);
		...
		case GLM_RIGHT_CLICKED:
			{
				...
				if( controlID == listboxIgnoreID ) 
				{
					...
					GPProfile profileID = (GPProfile)(IntPtr)GadgetListBoxGetItemData(control, rc->pos);
					UnicodeString nick = GadgetListBoxGetText(control, rc->pos);
					BuddyInfoMap *buddies = TheGameSpyInfo->getBuddyMap();
					BuddyInfoMap::iterator bIt;
