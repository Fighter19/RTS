		...
		case GLM_SELECTED:
			{
				...
				if ( controlID == GetGameListBoxID() )
				{
					...
					if( rowSelected >= 0 )
					{
						...
						req.stagingRoom.id = (IntPtr)GadgetListBoxGetItemData(control, rowSelected, 0);
		...
		case GBM_SELECTED:
			{
				...
				else if ( controlID == buttonJoinID )
				{
					...
					if (selected >= 0)
					{
						IntPtr selectedID = (IntPtr)GadgetListBoxGetItemData(GetGameListBox(), selected);
						if (selectedID > 0)
						{
		...
		case GCM_SELECTED:
			{
				...
				if( controlID == comboLobbyGroupRoomsID ) 
				{
					...
					{
						IntPtr groupID;
						groupID = (IntPtr)GadgetComboBoxGetItemData(comboLobbyGroupRooms, rowSelected);
		...
		case GLM_RIGHT_CLICKED:
			{
				...
				else if( controlID == GetGameListBoxID() ) 
				{
					...
					IntPtr selectedID = (IntPtr)GadgetListBoxGetItemData(control, rc->pos);
					if (selectedID > 0)
					{
						StagingRoomMap *srm = TheGameSpyInfo->getStagingRoomList();
						StagingRoomMap::iterator srmIt = srm->find(selectedID);
