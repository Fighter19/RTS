...
inline void BlitTrans<unsigned char>::BlitForward(void *dest, void const *source, int len) const {
	...
	again : __asm {dec ecx jz fini mov al, [esi] inc edi inc esi test al, al jz again mov[edi], al jmp again} fini :;
}
...
inline void BlitPlainXlat<unsigned short>::BlitForward(void *dest, void const *source, int len) const {
	...
	again :
		...
		*/
		__asm {
		xor eax, eax add edi, 2 mov al, [esi] inc esi mov ax, [ebx + eax * 2] mov[edi], ax dec ecx jnz again
	}
}
