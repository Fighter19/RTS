...
void RLEBlitTransZRemapXlat<unsigned short>::Blit(void *dest, void const *source, int len, int leadskip) const {
	...
	*/
	moreskip : __asm {test edx, edx jle nomoreskip dec edx lodsb test al, al jnz moreskip lodsb sub edx,
					  eax inc edx jmp moreskip} nomoreskip :
...
void RLEBlitTransXlat<unsigned short>::Blit(void *dest, void const *source, int len, int leadskip) const {
	...
	*/
	moreskip : __asm {test edx, edx jle nomoreskip dec edx lodsb test al, al jnz moreskip lodsb sub edx,
		...
		**	Output the pixel data to the destination.
		*/
		moredata : __asm {xor eax, eax or ecx, ecx jle fini lodsb test al, al jz transparent mov ax,
						  [ebx + eax * 2] dec ecx stosw jmp moredata}
