...
template <typename Event> class Notifier {
  public:
	...
	virtual bool HasObservers(void) const { return (mObservers.size() > 0); }
 ...
 */
template <typename Type, typename Value> class TypedEvent {
  public:
	TypedEvent(Value &value) : mValue(value) {}

	inline Value &operator()() { return mValue; }

	inline Value &Subject(void) { return mValue; }

  private:
	Value &mValue;
};

template <typename T, typename Object> class TypedEventPtr {
  public:
	TypedEventPtr(Object *subject) : mSubject(subject) {}

	inline Object *Subject(void) { return mSubject; }

	inline Object *operator()() { return mSubject; }
