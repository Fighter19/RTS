...
#else
	// TheSuperHackers @info Mauller 30/3/2020 All modern CPUs have the CPUID instruction, VS22 code will not run on a
	// cpu that doesn't.
...
void CPUDetectClass::Init_Processor_Log() {
	...
	if (CPUDetectClass::Get_L2_Cache_Size()) {
		SYSLOG(("L2 Cache: %d byte cache lines, %d way set associative, %dk\r\n",
				CPUDetectClass::Get_L2_Cache_Line_Size(), CPUDetectClass::Get_L2_Cache_Set_Associative(),
	...
	case VER_PLATFORM_WIN32_WINDOWS: {
		for (int i = 0; i < sizeof(Windows9xVersionTable) / sizeof(os_info); ++i) {
			if (Windows9xVersionTable[i].VersionMajor == OSVersionNumberMajor &&
				...
				Windows9xVersionTable[i].BuildMinor == build_minor && Windows9xVersionTable[i].BuildSub == build_sub) {
