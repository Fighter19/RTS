...
int main(int argc, char **argv)
{
	...
	DEBUG_LOG(("IN:'%s' OUT:'%s' Compression:'%s'", inFile.c_str(), outFile.c_str(),
	...
	if (outFile.empty())
	{
		...
		DEBUG_LOG(("'%s' is compressed using %s, from %d to %d bytes, %g%% of its original size", inFile.c_str(),
				   CompressionManager::getCompressionNameByType(usedType), uncompressedSize, size,
